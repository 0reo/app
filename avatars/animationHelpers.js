  ...
  const _getApplyFn = () => {
    ...
    if (avatar.danceTime > 0) {
      return spec => {
        ...
        // console.log('dance time', avatar.danceTime, t2);
        // dst.fromArray(v2);
    ...
    /* if (avatar.standChargeState) {
      return spec => {
        ...
        const t2 = (avatar.standChargeTime/1000) ;
    ...
    if (avatar.swordSideSlashState) {
      return spec => {
        ...
        const t2 = (avatar.swordSideSlashTime/1000) ;
    ...
    if (avatar.swordTopDownSlashState) {
      return spec => {
        const {
          animationTrackName: k,
          dst,
        ...
        const t2 = (avatar.swordTopDownSlashTime/1000) ;
    ...
    /* if (avatar.chargeJumpState) {
      return spec => {
        ...
        const t2 = (avatar.chargeJumpTime/1000) ;
    ...
    /* if (avatar.throwState) {
      return spec => {
        ...
        const throwAnimation = throwAnimations[avatar.throwAnimation || defaultThrowAnimation];
        const src2 = throwAnimation.interpolants[k];
        const t2 = avatar.throwTime/1000;
        const v2 = src2.evaluate(t2);

        dst.fromArray(v2);
      };
    } */
    // console.log('got aim time', avatar.useAnimation, avatar.useTime, avatar.aimAnimation, avatar.aimTime);
    if (avatar.useAnimation) {
      return spec => {
        ...
        const isCombo = Array.isArray(avatar.useAnimation);
        const useAnimationName = isCombo ? avatar.useAnimation[avatar.useAnimationIndex] : avatar.useAnimation;
        const useAnimation = (useAnimationName && useAnimations[useAnimationName]);
        _handleDefault(spec);
        const t2 = Math.min(avatar.useTime / 1000, useAnimation.duration); /* (() => {
              if (isCombo) {
                return Math.min(avatar.useTime/1000, useAnimation.duration);
              } else {
                return (avatar.useTime/1000) % useAnimation.duration;
              }
            })(); */
        if (!isPosition) {
          if (useAnimation) {
          ...
          _clearXZ(avatar.localVector2, isPosition);
